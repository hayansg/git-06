...
- cascading 규칙
- 동일한 대상에 여러 스타일이 적용할 때 제일 마지막에 적용된 스타일에 반영
...
##### width/height(block 요소)
  - block 요소 
    - width는 부모요소에 채워짐
    - heigh는 contents 또는 자식요소에 맞춰짐
  - px
    - 수지 값으로 크기 조정
  - %
    - 부모 요소를 기준으로일정 비율 크기만큼 지정
    - height는 적용이 되지 않음
- auto
...
display:block; /* block 요소가 block요소의 특성으로 화면에 표시 */
